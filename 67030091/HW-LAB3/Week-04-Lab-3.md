...
2. **คำถามเชิงวิเคราะห์ (Engineering Evaluation):**
    ...
    ข้อ 1: SD พัฒนาไปในทางที่ดีขึ้นอย่างไร

    ในใบงานที่ 2 ระบบอ่านค่าดิบเพียงจุดเดียวขณะที่สัญญาณยังอยู่ในช่วง Transient State ทำให้ค่าที่ได้ผันผวนสูงมาก ขึ้นอยู่กับจังหวะเวลาที่สุ่มอ่านล้วน ๆ ส่วนในใบงานที่ 3 ระบบเปลี่ยนมาสุ่มเก็บ 50 แซมเปิ้ล แล้วทำ Sorting ตัดข้อมูลขอบบน-ล่างออกฝั่งละ 10% ก่อนหาค่าเฉลี่ย (Trimmed Mean) ทำให้ Noise แบบสุ่มหักล้างกันเองตามหลักค่าเฉลี่ยเลขคณิต และ Outlier จาก Spike Noise ถูกคัดออกไปก่อน ส่งผลให้ค่า SD ลดลงอย่างชัดเจนเมื่อสภาพแวดล้อมนิ่ง สะท้อนว่าข้อมูลมีความน่าเชื่อถือและเสถียรมากขึ้นกว่าการอ่านค่าดิบตรง ๆ
    ...
    ข้อ 2: ทำไมต้องใช้หน่วย mV แทน Raw Scale (0-4095)

    ตามหลักการ "สองโลกที่แตกต่าง" ค่า Raw เป็นเพียงตัวเลขบิตในโลกดิจิทัลที่ผูกติดกับคุณสมบัติเฉพาะของ ADC แต่ละตัว (ความละเอียด, Reference Voltage, ความคลาดเคลื่อนจากการผลิต) จึงไม่มีความหมายทางกายภาพที่แน่นอน หากเปลี่ยนบอร์ดหรือชิป ค่าตัวเลขเดียวกันอาจสื่อถึงความเข้มแสงที่ต่างกัน แต่หน่วย mV คือปริมาณทางไฟฟ้าจริงที่ผ่านการ Calibrate ด้วยโมดูล eFuse Line Fitting มาแล้ว จึงเป็นตัวแทนของโลกกายภาพที่แม่นยำและเทียบข้ามอุปกรณ์ได้ การนำไปตั้งค่า Threshold ควบคุมหุ่นยนต์แยกสีจึงเสถียรและแม่นยำกว่าการใช้ค่าดิบที่ผูกติดกับฮาร์ดแวร์เฉพาะตัว