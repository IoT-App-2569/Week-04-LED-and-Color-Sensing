...
Color G, n = 40 (filtered), mean = 183.00, sd = 25.00
Color B, n = 40 (filtered), mean = 184.00, sd = 8.00
--------------------------------------------------------------
Color R, n = 40 (filtered), mean = 182.00, sd = 16.00
Color G, n = 40 (filtered), mean = 184.00, sd = 21.00
Color B, n = 40 (filtered), mean = 183.00, sd = 17.00
--------------------------------------------------------------
Color R, n = 40 (filtered), mean = 176.00, sd = 23.00
Color G, n = 40 (filtered), mean = 175.00, sd = 17.00
Color B, n = 40 (filtered), mean = 183.00, sd = 7.00
--------------------------------------------------------------
Color R, n = 40 (filtered), mean = 176.00, sd = 21.00
Color G, n = 40 (filtered), mean = 188.00, sd = 25.00
Color B, n = 40 (filtered), mean = 182.00, sd = 12.00
--------------------------------------------------------------
Color R, n = 40 (filtered), mean = 175.00, sd = 8.00
Color G, n = 40 (filtered), mean = 182.00, sd = 5.00
...
ในใบงานที่ 2 ระบบใช้วิธีการอ่านค่าดิบเพียงครั้งเดียว (Single Sampling) ในจังหวะที่สัญญาณอาจจะยังไม่เข้าสู่สภาวะคงที่ (Transient State) จึงทำให้ค่าที่ได้มีความผันผวนสูงมาก ขึ้นอยู่กับจังหวะเวลาที่สุ่มอ่านได้ 

แต่ในใบงานที่ 3 ได้มีการปรับปรุงสถาปัตยกรรมโดยการดึงข้อมูลมา 50 แซมเปิ้ล นำมาเรียงลำดับ (Sorting) และทำการตัดข้อมูลส่วนหัวและท้ายออกฝั่งละ 10% เพื่อกำจัดค่าที่ผิดปกติ (Outlier) หรือสัญญาณรบกวนแบบฉับพลัน (Spike Noise) ออกไปเสียก่อน จากนั้นจึงนำข้อมูลที่เหลือมาหาค่าเฉลี่ย (Trimmed Mean) กระบวนการนี้ช่วยให้สัญญาณรบกวนแบบสุ่ม (Random Noise) ถูกหักล้างกันไปตามหลักคณิตศาสตร์ ส่งผลให้ค่าส่วนเบี่ยงเบนมาตรฐาน (SD) ลดต่ำลงอย่างเห็นได้ชัด ข้อมูลที่ได้จึงมีความเสถียร นิ่ง และเชื่อถือได้มากกว่าการดึงค่าดิบมาใช้โดยตรง
...
เมื่อพิจารณาตามแนวคิด "สองโลกที่แตกต่าง" ค่า Raw Scale (0-4095) ถือเป็นเพียงข้อมูลใน "โลกดิจิทัล" ที่ยึดโยงอยู่กับคุณสมบัติทางฮาร์ดแวร์ของ ADC บนบอร์ดไมโครคอนโทรลเลอร์แต่ละตัว (เช่น ค่าแรงดันอ้างอิง, ความละเอียด, หรือความคลาดเคลื่อนเฉพาะตัว) ซึ่งไม่ได้สะท้อนความหมายทางกายภาพที่แท้จริง หากนำโค้ดไปรันบนบอร์ดตัวอื่น ค่า Raw ที่อ่านได้ก็อาจผิดเพี้ยนไปแม้ความเข้มแสงจะเท่าเดิมก็ตาม

ในทางกลับกัน การแปลงข้อมูลให้อยู่ในหน่วยมิลลิโวลต์ (mV) คือการนำข้อมูลกลับเข้าสู่ "โลกกายภาพ" ซึ่งเป็นปริมาณทางไฟฟ้าที่เกิดขึ้นจริงและผ่านกระบวนการสอบเทียบ (Calibration) ด้วยเทคนิค eFuse Line Fitting มาแล้ว การใช้หน่วย mV ในการกำหนดค่าเกณฑ์ (Threshold) เพื่อควบคุมหุ่นยนต์ จึงมีความแม่นยำ เสถียรภาพสูง และได้มาตรฐานเดียวกัน ทำให้สามารถนำระบบนี้ไปใช้งานข้ามอุปกรณ์ (Cross-Hardware) ได้โดยไม่เกิดความผิดเพี้ยน

### 🎥 วิดีโอผลการทดลอง
[คลิกเพื่อรับชมวิดีโอผลการทดลอง Lab 3](https://youtube.com/shorts/mzsfhitdLAg?si=BPLNynzpt3uQlwpn)